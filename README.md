...
### 5) 같은 네트워크의 다른 기기에서 접속

`make api`(0.0.0.0:8000) 와 `make web`(0.0.0.0:3000) 은 LAN 에 열려 있다. 같은 공유기에
붙은 노트북·휴대폰에서 아래 주소로 열면 된다.

```bash
ipconfig getifaddr en0        # macOS: 이 컴퓨터의 LAN IP (예: 192.168.219.105)
# 다른 기기 브라우저에서 http://<이 컴퓨터의 LAN IP>:3000
```

화면은 `NEXT_PUBLIC_API_URL` 이 없으면 **페이지를 연 호스트의 8000 포트**를 API 로 쓴다.
그래서 LAN IP 로 열어도 API 호출이 그 기기의 localhost 로 새지 않는다. 백엔드는
`WEB_ORIGIN_REGEX` 기본값으로 사설망 출처(`192.168.*`·`10.*`·`172.16~31.*`)를 CORS 허용한다.

- **접속이 안 되면 macOS 방화벽**: 시스템 설정 → 네트워크 → 방화벽 → 옵션에서
  `node` 와 `python`(uv 가 쓰는 인터프리터)의 **들어오는 연결 허용**을 켠다.
  처음 서버를 띄울 때 뜨는 "수신 연결을 허용하시겠습니까?" 팝업에서 **허용**을 눌러도 된다.
- **API 주소를 강제로 지정**하려면 `apps/web/.env.local` 에
  `NEXT_PUBLIC_API_URL=http://192.168.219.105:8000` 처럼 적는다(설정하면 그 값이 우선한다).
- 사설망 CORS 허용을 끄려면 `.env` 에 `WEB_ORIGIN_REGEX=` 를 비워 두고
  `WEB_ORIGINS` 명시 목록만 쓴다(운영 배포 권장).
